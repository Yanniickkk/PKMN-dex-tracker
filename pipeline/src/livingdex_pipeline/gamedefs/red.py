"""Pokemon Red: everything but the smoke test.
...
This is the Virtual Console release rather than the 1996 cartridge, and it is the only Red in
this dataset - see :mod:`vc` for why. Everything it shares with Blue is in :mod:`kanto`, which
...
ONLY_ON_BLUE: dict[str, str | None] = {
    "sandshrew": None,
    "vulpix": None,
    "meowth": None,
    "bellsprout": None,
...
UNOBTAINABLE: dict[str, str] = {
    ...
    **{species: kanto.gb_only_on("Blue", event) for species, event in ONLY_ON_BLUE.items()},
}
...
def dex_entries(context: BuildContext) -> list[DexEntry]:
    ...
    Here it is the whole of it. A living dex in Red is 151 tiles, not 151 of something
...
def register(registry: GameRegistry) -> None:
    registry.register(GAME_ID, build, edges(), box_art="Red EN boxart.png")
