"""Pokemon FireRed on the Nintendo Switch: the cartridge again, with one door instead of another.
...
and the unobtainable list are :mod:`firered`'s, imported rather than retyped: two copies of a
...
from . import firered, frlg_switch, kanto

GAME_ID = "firered-switch"
...
#: cable.
PAIR_PARTNER = "leafgreen-switch"

#: What PokeAPI calls this game, which is what it called it before. The source has one FireRed
#: and this is a re-release of it, so asking about `firered` is asking about this.
POKEAPI_VERSION = firered.POKEAPI_VERSION
...
def build(context: BuildContext) -> GameData:
    ...
    return GameData(
        game=frlg_switch.release(
            game_id=GAME_ID,
            title="Pokémon FireRed Version (Nintendo Switch)",
            version="FireRed",
...
def dex_entries(context: BuildContext) -> list[DexEntry]:
    ...
    return kanto.dex_entries(context, game_id=GAME_ID, unobtainable=firered.UNOBTAINABLE)
