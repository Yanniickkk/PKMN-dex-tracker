"""Pokemon Violet: what it is, and the three routes it brings.
...
**The half whose future is the exclusive.** The Iron Paradox Pokemon are here - Iron Treads,
Iron Bundle, Iron Hands, Iron Jugulis, Iron Moth, Iron Thorns, Iron Valiant and Miraidon - where
Scarlet has the ancient ones, and the professor goes with them: Turo here and Sada there,
Arven's father against his mother.

What is left for this file is its name, that Scarlet is its other half, which version PokeAPI is
...
def dex_entries(context: BuildContext) -> list[DexEntry]:
    ...
    The same three lists Scarlet shows, with the same numbers, which is what a version pair
...
def edges() -> list[TransferEdge]:
    """The cable to Scarlet, and HOME in both directions.
...
def register(registry: GameRegistry) -> None:
    registry.register(GAME_ID, build, edges(), box_art="Violet EN boxart.png")
